package cl.yerkodee.ionix_test.ui.common;
...
public abstract class DataBoundListAdapter<T, V extends ViewDataBinding>
    ...
    private List<T> itemsFiltered = new ArrayList<>();
    ...
    public final void onBindViewHolder(DataBoundViewHolder<V> holder, int position) {
        bind(holder.binding, itemsFiltered.get(position));
    ...
    @MainThread
    public void replace(List<T> updateItems) {
        dataVersion++;
        if (itemsFiltered == null) {
            if (updateItems == null) {
                return;
            }
            itemsFiltered = updateItems;
            notifyDataSetChanged();
        } else if (updateItems == null) {
            int oldSize = itemsFiltered.size();
            itemsFiltered = null;
            notifyItemRangeRemoved(0, oldSize);
        } else {
            final int startVersion = dataVersion;
            final List<T> oldItems = itemsFiltered;
                ...
                protected DiffUtil.DiffResult doInBackground(Void... voids) {
                    return DiffUtil.calculateDiff(new DiffUtil.Callback() {
                        ...
                        public boolean areItemsTheSame(int oldItemPosition, int newItemPosition) {
                            ...
                            T newItem = updateItems.get(newItemPosition);
